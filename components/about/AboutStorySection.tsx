...
/** About page story block — mirrors homepage `StorySection` layout & copy; image from `aboutPage.story`. */
export function AboutStorySection() {
  const { story } = siteContent;
  const img = siteContent.aboutPage.story;

  return (
    <section
      id="about-story"
      aria-labelledby="about-story-heading"
      className="scroll-mt-24 bg-exit-warm px-5 py-16 md:px-[60px] md:py-20"
    >
      <div className="mx-auto grid max-w-[1100px] grid-cols-1 items-center gap-12 md:grid-cols-2 md:gap-20">
        <div className="relative h-[280px] overflow-hidden rounded-[20px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] md:h-[400px]">
          <img
            src={img.imageSrc}
            alt={img.imageAlt}
            className="h-full w-full rounded-[inherit] object-cover object-[center_40%]"
          />
        ...
        <div>
          <p className="mb-2.5 text-[11px] font-bold uppercase tracking-[2px] text-exit-green">{story.eyebrow}</p>
          <span className="font-hand mb-2 block rotate-[-1deg] text-[22px] text-exit-green">
            {story.handwrittenLine}
          </span>
          <h2
            id="about-story-heading"
            className="font-display mb-5 text-[clamp(2.25rem,4vw,3.25rem)] font-black uppercase leading-[0.95] tracking-[-0.5px] text-exit-dark md:mb-5"
          >
