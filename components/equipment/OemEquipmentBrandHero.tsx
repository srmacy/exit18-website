...
/** Clone of EchoPageHero — props drive brand imagery + copy; Echo route stays on Echo-only component. */
export function OemEquipmentBrandHero({ content: p, ui }: Props) {
  const hasHero = publicImageExists(ui.heroImage);
  ...
  return (
    <section
      aria-labelledby={`${ui.slug}-page-hero-heading`}
        ...
        <div className="min-w-0 lg:col-span-6">
          <div className="overflow-hidden rounded-2xl border border-exit-dark/[0.07] shadow-[0_20px_60px_rgba(0,0,0,0.35)] ring-1 ring-black/[0.04]">
            {hasHero ? (
              ...
              <OemBrandHeroPlaceholder hint={ui.heroPlaceholderHint} aspectClass="min-h-[220px] sm:min-h-[260px]" />
            )}
          </div>
        </div>
      </div>
...
function OemBrandHeroPlaceholder({ hint, aspectClass }: { hint: string; aspectClass: string }) {
  return (
    <div
